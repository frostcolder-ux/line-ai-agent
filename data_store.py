...
ESG_PATH = os.path.join(BASE_DIR, "esg_status.json")

_DEFAULT_ESG = {
    "苗栗自閉症協進會": {"contract": False, "image_rights": False, "note": ""},
    "芥菜種會": {"contract": False, "image_rights": False, "note": ""},
    "臺東康復之友協會": {"contract": False, "image_rights": False, "note": ""},
    "靜鴻家園": {"contract": False, "image_rights": False, "note": ""},
    "亞葵小鎮": {"contract": False, "image_rights": False, "note": ""},
}


def get_esg_status() -> dict:
    if _use_pg():
        from db import get_conn
        conn = get_conn()
        cur = conn.cursor()
        cur.execute("SELECT organization, contract_signed, image_rights_signed, note FROM esg_documents")
        rows = cur.fetchall()
        cur.close(); conn.close()
        if not rows:
            return _DEFAULT_ESG.copy()
        return {r[0]: {"contract": r[1], "image_rights": r[2], "note": r[3] or ""} for r in rows}
    else:
        try:
            with open(ESG_PATH, encoding="utf-8") as f:
                return json.load(f)
        except (FileNotFoundError, json.JSONDecodeError):
            data = _DEFAULT_ESG.copy()
            with open(ESG_PATH, "w", encoding="utf-8") as f:
                json.dump(data, f, ensure_ascii=False, indent=2)
            return data


def update_esg_status(organization: str, doc_type: str, signed: bool, note: str = "") -> dict:
    now = datetime.now().strftime("%Y-%m-%d %H:%M")
    if _use_pg():
        from db import get_conn
        conn = get_conn()
        ...
        if doc_type in ("contract", "both"):
            cur.execute(
                "INSERT INTO esg_documents (organization, contract_signed, image_rights_signed, note, updated_at) "
                "VALUES (%s, %s, %s, %s, %s) "
                "ON CONFLICT (organization) DO UPDATE SET contract_signed=%s, note=%s, updated_at=%s",
                (organization, signed, False, note, now, signed, note, now)
            )
        if doc_type in ("image_rights", "both"):
            cur.execute(
                "INSERT INTO esg_documents (organization, contract_signed, image_rights_signed, note, updated_at) "
                "VALUES (%s, %s, %s, %s, %s) "
                "ON CONFLICT (organization) DO UPDATE SET image_rights_signed=%s, note=%s, updated_at=%s",
                (organization, False, signed, note, now, signed, note, now)
            )
        if doc_type == "both":
            cur.execute(
                "UPDATE esg_documents SET contract_signed=%s, image_rights_signed=%s, note=%s, updated_at=%s "
                "WHERE organization=%s",
                (signed, signed, note, now, organization)
            )
        conn.commit(); cur.close(); conn.close()
        log(f"PG ESG update: {organization} {doc_type}={signed}")
        return {"organization": organization, "doc_type": doc_type, "signed": signed, "note": note}
    else:
        data = get_esg_status()
        if organization not in data:
            data[organization] = {"contract": False, "image_rights": False, "note": ""}
        if doc_type in ("contract", "both"):
            data[organization]["contract"] = signed
        if doc_type in ("image_rights", "both"):
            data[organization]["image_rights"] = signed
        if note:
            data[organization]["note"] = note
        with open(ESG_PATH, "w", encoding="utf-8") as f:
            json.dump(data, f, ensure_ascii=False, indent=2)
